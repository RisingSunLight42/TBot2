...
export const edtDuJour = async (client: ClientExtend) => {
    ...
    const edtDataTP1Asked = await dataEdtProcessing(edtDataTP1, 0, false);
    const edtDataTP2 = await fetchEdt(1186);
    const edtDataTP2Asked = await dataEdtProcessing(edtDataTP2, 0, false);
    await channel.send({
